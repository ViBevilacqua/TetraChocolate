...
//MDN
                ...
                if (item.textureIndex > 0 && !this->noTexture) { // Texture coordinates are provided
                    int index = item.textureIndex;
                    if (item.textureIndex > object_.textures.size()-1) index = object_.textures.size()-1;
                    GLfloat  x = object_.textures.at(index).coords[0];
                    GLfloat  y = object_.textures.at(index).coords[1];
                    if (!noTexture)
                        glTexCoord2f(x * material.texScaleU,y * material.texScaleV);
...
//                       object_.textures.at(item.textureIndex).coords[1] * material.texScaleV);
    ...
    void Model::renderBoundingBox() const
    {
        ...
        glColor4f(1, 0, 0, 1);
        static const float blue[] = { 0.13333, 0.5764705882352941, 0.803921568627451, 1 };
